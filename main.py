...
async def clear_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.chat_data['history'] = []  # История без системной роли
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    use_search = user_search_enabled.get(chat_id, True) and model_id.startswith('gemini-2.5')  # Поиск только для 2.5
    ...
    try:
        ...
        tools = [
            {
                'google_search_retrieval': {'dynamic_threshold': 0.5}
            }
        ...
        while total_chars > MAX_CONTEXT_CHARS and len(chat_history) > 1:
            chat_history.pop(0)  # Удаляем старые сообщения
            total_chars = sum(len(p['parts'][0]['text']) for p in chat_history)
