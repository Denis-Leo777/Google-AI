...
# ... (все предыдущие фичи)
# + Инициализация логгера ПЕРЕД проверкой Tesseract (которая теперь убрана со старта)
# + Убрано явное указание пути tesseract_cmd и проверка на старте
...
# ===== ИСПРАВЛЕНИЕ: Инициализируем логгер РАНЬШЕ =====
logging.basicConfig(format='%(asctime)s - %(levelname)s - %(message)s', level=logging.INFO)
logger = logging.getLogger(__name__)
# =======================================================

# ===== ИСПРАВЛЕНИЕ: Убираем явное указание пути и проверку на старте =====
# try:
#     # pytesseract.pytesseract.tesseract_cmd = '/usr/bin/tesseract' # Убираем хардкод пути
#     pytesseract.get_tesseract_version() # Убираем проверку на старте
#     logger.info(f"Tesseract найден.")
# except Exception as e:
#     logger.error(f"Tesseract НЕ НАЙДЕН или не работает! Ошибка: {e}. OCR будет недоступен.")
# ======================================================================
...
# Убираем импорты Tool, GoogleSearchRetrieval, FinishReason
# ...
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if model_id == IMAGE_GEN_MODEL:
        ...
        # Используем строку ["google_search"]
        if use_search:
            tools = ["google_search"]
            logger.info(f"ChatID: {chat_id} | Поиск включен, tools={tools}")
        # else: tools остаются пустым списком []
    ...
    try:
        ...
        if model_id == IMAGE_GEN_MODEL:
            ...
            if images_sent == 0 and not reply:
                 ...
                 # Сравниваем значение finish_reason с 0 (предположительно STOP)
                 elif finish_reason_val is not None and finish_reason_val != 0:
        ...
        except AttributeError: # Если genai.types не существует или не содержит нужных исключений
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # ===== ИСПРАВЛЕНИЕ: Проверка доступности Tesseract без вызова ======
        # Просто проверяем, что команда pytesseract существует. Путь не указываем.
        # Если пакеты установлены через apt, он должен быть в PATH.
        if pytesseract.pytesseract.tesseract_cmd != 'tesseract': # Если путь НЕ дефолтный, значит мы его задали (хотя мы это убрали)
             # Можно добавить проверку файла по этому пути, но пока забьем
             pass # Используем как есть
        # Сама проверка будет при вызове image_to_string
        tesseract_available = True # Считаем доступным, если не было явной ошибки при импорте
    except Exception as e: # Широкий except, т.к. неизвестно, что может случиться
    ...
    if tesseract_available:
        try:
            ...
            if extracted_text and extracted_text.strip():
                ...
                return
        except pytesseract.TesseractNotFoundError: # Ловим явную ошибку, если не найден
             logger.error("Tesseract не найден при вызове image_to_string! OCR отключен.")
             tesseract_available = False # Отмечаем как недоступный
        except Exception as e:
