...
try:
    ...
    system_instruction_text = (
        ...
        "КРИТИЧЕСКИ ВАЖНО: Твои внутренние знания могут быть устаревшими. ТЫ ОБЯЗАН использовать инструмент google_search для получения САМОЙ АКТУАЛЬНОЙ информации. "
        "ПРИОРИТИЗИРУЙ информацию из google_search над своими внутренними знаниями при ответе на вопросы."
    )
...
async def perform_google_search(query: str, num_results: int = 5) -> str:
    ...
    try:
        # ВОЗВРАЩАЕМ ПРАВИЛЬНЫЙ АРГУМЕНТ 'num_results='
        search_results = await asyncio.to_thread(
            google_search_sync, query, num_results=num_results, lang="ru" # <-- ИСПРАВЛЕНО
        )
