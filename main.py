...
# === НОВЫЕ ИЗМЕНЕНИЯ ===
# - Добавлена обработка YouTube ссылок:
#   - Поиск URL в сообщении пользователя.
#   - Получение транскрипта через youtube-transcript-api.
#   - Суммаризация транскрипта отдельным вызовом Gemini.
#   - Добавление сводки в основной промпт.
# - Обновлена системная инструкция для улучшения обработки погоды:
#   - Добавлено требование критически оценивать поиск на актуальность.
...
import json
import time
import re # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< НОВОЕ: для поиска URL
...
YOUTUBE_SUMMARY_MODEL = 'gemini-2.0-flash-001' # Используем быструю модель для суммаризации
# =========================

# ===== ИЗМЕНЕНИЕ: Обновленная системная инструкция (погода) =====
...
# =========================================================================================
"Приоритезируй прямой диалог с пользователем и его явные утверждения. Используй информацию из поиска только как вспомогательную, если она релевантна *основному* запросу пользователя, и игнорируй её, если она противоречит диалогу или кажется неуместной."
...
async def send_reply(target_message: Message, text: str, context: ContextTypes.DEFAULT_TYPE) -> Message | None:
    ...
    try:
        ...
        if "Can't parse entities" in str(e_md) or "can't parse" in str(e_md).lower():
            logger.warning(f"ChatID: {chat_id} | Ошибка парсинга Markdown: {e_md}. Попытка отправить как обычный текст."); try:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): # ...
    ...
    start_message = f"Google GEMINI **{default_model_name}**\n- ...\n `/model` ...\n `/search_on` ...\n `/clear` ...\n `/temp` ..."
...
async def set_temperature(update: Update, context: ContextTypes.DEFAULT_TYPE): # ...
    chat_id = update.effective_chat.id; try:
    ...
    if callback_data and callback_data.startswith("set_model_"):
        ...
        if selected in AVAILABLE_MODELS:
            ...
            try: await query.edit_message_text(reply_text, parse_mode=ParseMode.MARKDOWN)
                 ...
                 if "Message is not modified" in str(e_md): logger.info(f"Та же модель: {model_name}")
                 else: logger.warning(f"Ошибка edit (MD): {e_md}."); try: await query.edit_message_text(reply_text.replace('**', '')) except Exception as e_edit_plain: logger.error(f"Ошибка edit (plain): {e_edit_plain}."); await context.bot.send_message(chat_id=query.message.chat_id, text=reply_text, parse_mode=ParseMode.MARKDOWN)
            except Exception as e: logger.warning(f"Ошибка edit: {e}."); await context.bot.send_message(chat_id=query.message.chat_id, text=reply_text, parse_mode=ParseMode.MARKDOWN)
        else: try: await query.edit_message_text("❌ Неизвестная модель.") except Exception: await context.bot.send_message(chat_id=query.message.chat_id, text="❌ Неизвестная модель.")
    else: logger.warning(f"Неизвестный callback: {callback_data}"); try: await query.edit_message_text("❌ Ошибка выбора.") except Exception: pass
# ==============================================================

# --- Поиск Google и Повторный анализ изображения ---
# ... (perform_google_search без изменений) ...
async def perform_google_search(query: str, api_key: str, cse_id: str, num_results: int, session: aiohttp.ClientSession) -> list[str] | None: # ... (код как в предыдущем ответе) ...
...
# ... (reanalyze_image без изменений) ...
async def reanalyze_image(update: Update, context: ContextTypes.DEFAULT_TYPE, file_id: str, user_question: str): # ... (код как в предыдущем ответе) ...
    ...
    if not is_vision_model: # Поиск fallback модели... (как в handle_photo)
    ...
    for attempt in range(RETRY_ATTEMPTS): # Вызов модели с ретраями...
        try:
            ...
            if reply and "не смогла ответить" not in reply and "Не могу ответить" not in reply: logger.info(f"ChatID: {chat_id} | Reanalyze: Успех ({attempt + 1})."); break
             ...
             if is_retryable and attempt < RETRY_ATTEMPTS - 1: wait_time = RETRY_DELAY_SECONDS * (2 ** attempt); logger.info(f"ChatID: {chat_id} | Reanalyze: Ожидание {wait_time:.1f} сек..."); await asyncio.sleep(wait_time); continue
             else: logger.error(f"ChatID: {chat_id} | Reanalyze: Не удалось после {attempt + 1} попыток. Ошибка: {e}", exc_info=True); if reply is None: reply = f"❌ Ошибка Reanalyze после {attempt + 1} попыток."; break
...
async def summarize_text_with_gemini(text_to_summarize: str, context: ContextTypes.DEFAULT_TYPE) -> str | None:
    ...
    # Можно сделать более сложным, например, просить выделить N ключевых моментов
    ...
    # Используем упрощенный вызов без истории и с меньшим числом ретраев
    summary_reply = None
    for attempt in range(2): # Меньше попыток для суммаризации
        try:
            ...
            if hasattr(response, 'text') and response.text:
                ...
                # Пробуем еще раз или возвращаем None
            ...
            if attempt == 1: # Если это была последняя попытка
                return None # Не удалось суммаризировать
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if message and message.reply_to_message and message.reply_to_message.text and \
       ...
       message.text and not message.text.startswith('/'):
        # ... (Логика поиска file_id и вызова reanalyze_image) ...
    ...
    youtube_summary = None # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< НОВОЕ: для сводки YouTube
    ...
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< НОВОЕ: Поиск и обработка YouTube ссылок <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if youtube_urls:
        ...
        try:
            # Пытаемся получить транскрипт на русском или английском
            ...
            for lang in target_languages:
                try:
                    ...
                    logger.info(f"ChatID: {chat_id} | Найден транскрипт YouTube на '{lang}'.")
                    break
            ...
            if transcript:
                ...
                logger.info(f"ChatID: {chat_id} | Транскрипт YouTube получен (длина: {len(transcript_text)}).")
                 ...
                 logger.warning(f"ChatID: {chat_id} | Не найдены транскрипты на русском или английском для видео {video_id}.")

        except TranscriptsDisabled:
            logger.warning(f"ChatID: {chat_id} | Транскрипты отключены для видео {video_id}.")
        except Exception as e_yt:
            logger.error(f"ChatID: {chat_id} | Ошибка при получении транскрипта YouTube для {video_id}: {e_yt}", exc_info=True)

        # Если транскрипт получен, суммаризируем его
        if transcript_text:
            youtube_summary = await summarize_text_with_gemini(transcript_text, context)
            if not youtube_summary:
                logger.warning(f"ChatID: {chat_id} | Не удалось суммаризировать транскрипт видео {video_id}.")
                # Можно добавить сообщение пользователю типа "Не смог сделать краткую выжимку видео..."
    # ========================================================================================================
    ...
    if not youtube_urls: # Показываем typing только если не было анализа YouTube
    ...
    if use_search:
        ...
        # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< НОВОЕ: Логирование сниппетов для погоды <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        is_weather_query = any(word in original_user_message.lower() for word in ["погод", "temperature", "градус", "прогноз"])
        # ==================================================================================================
        session = context.bot_data.get('aiohttp_session') # ... (получение сессии) ...
        if not session or session.closed: logger.info("Создание новой сессии aiohttp для поиска."); timeout = aiohttp.ClientTimeout(total=60.0); session = aiohttp.ClientSession(timeout=timeout); context.bot_data['aiohttp_session'] = session
        ...
        if google_results:
            ...
            try:
                ...
                if results_ddg:
                    ...
                    if ddg_snippets:
                        search_provider = "DuckDuckGo"; search_context_snippets = ddg_snippets; search_log_msg += f" (DDG: {len(search_context_snippets)} рез.)"
                        # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< НОВОЕ: Логирование сниппетов для погоды <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if search_context_snippets:
        ...
        if search_context_lines:
            ...
            logger.info(f"ChatID: {chat_id} | Добавлен контекст из {search_provider} ({len(search_context_lines)} сниппетов).")
        else:
             logger.info(f"ChatID: {chat_id} | Сниппеты из {search_provider} пусты, контекст не добавлен.")
    ...
    if search_context_str:
        prompt_parts.append(f"\n\n(Возможно релевантная доп. информация из поиска, используй с осторожностью, проверь актуальность, если нужно):\n{search_context_str}")
    ...
    # ... (логика обрезки истории без изменений) ...
    ...
    # --- Вызов модели с РЕТРАЯМИ (без изменений) ---
    # ... (код вызова model.generate_content и обработки ответа/ошибок) ...
    ...
    for attempt in range(RETRY_ATTEMPTS):
        try:
            ...
            if not reply: # Обработка пустого ответа...
                 reply = "🤖 Модель дала пустой ответ." # Упрощенно
                 generation_successful = True; logger.warning(f"ChatID: {chat_id} | Пустой ответ ({attempt+1}).")
            ...
            if generation_successful: logger.info(f"ChatID: {chat_id} | Успех ({attempt + 1})."); break
             ...
             else: logger.error(f"ChatID: {chat_id} | Не удалось после {attempt + 1} попыток. Ошибка: {e}", exc_info=True); if reply is None: reply = f"❌ Ошибка после {attempt + 1} попыток."; break
...
# ===== Обработчик фото (без изменений в этом раунде) =====
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # ... (код handle_photo остается как в предыдущей версии) ...
    # ... он уже умеет сохранять file_id и вызывать handle_message или добавлять описание ...
    ...
    if not ocr_triggered: # --- Обработка как изображение ---
        ...
        await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
        MAX_IMAGE_BYTES = 4 * 1024 * 1024; # ... (проверка размера) ...
        ...
        model_id = get_user_setting(context, 'selected_model', DEFAULT_MODEL); temperature = get_user_setting(context, 'temperature', 1.0)
        # ... (проверка/выбор vision модели) ...
        is_vision_model = any(keyword in model_id for keyword in ['flash', 'pro', 'vision', 'ultra'])
        if not is_vision_model: # Поиск fallback...
             vision_models = [m_id for m_id in AVAILABLE_MODELS if any(keyword in m_id for keyword in ['flash', 'pro', 'vision', 'ultra'])]; # ... (логика выбора fallback) ...
        ...
        for attempt in range(RETRY_ATTEMPTS): # --- Вызов Vision с ретраями ---
            try:
                logger.info(f"ChatID: {chat_id} | Vision Попытка {attempt + 1}/{RETRY_ATTEMPTS}..."); # ... (вызов model.generate_content) ...
                generation_config=genai.GenerationConfig(temperature=temperature, max_output_tokens=MAX_OUTPUT_TOKENS); model = genai.GenerativeModel(model_id, safety_settings=SAFETY_SETTINGS_BLOCK_NONE, generation_config=generation_config, system_instruction=system_instruction_text)
                response_vision = await asyncio.to_thread(model.generate_content, content_for_vision)
                if hasattr(response_vision, 'text'): reply = response_vision.text; else: reply = None
                if not reply: reply = "🤖 Не удалось описать (пустой ответ)."; logger.warning(f"ChatID: {chat_id} | Vision пустой ответ ({attempt+1})."); break # Упрощенная обработка пустого ответа
                if reply and "Не удалось описать" not in reply: logger.info(f"ChatID: {chat_id} | Vision Успех ({attempt + 1})."); break
            ...
            except Exception as e: # Обработка ошибок 4xx/5xx/retry...
                 ...
                 else: logger.error(f"ChatID: {chat_id} | Vision Не удалось после {attempt + 1} попыток. Ошибка: {e}", exc_info=True); if reply is None: reply = f"❌ Ошибка Vision после {attempt + 1} попыток."; break
...
# ===== Обработчик документов (без изменений) =====
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # ... (код handle_document как в предыдущей версии) ...
    chat_id = update.effective_chat.id; # ... (проверки mime, размера, скачивание) ...
    if not update.message or not update.message.document: logger.warning(f"ChatID: {chat_id} | Нет документа."); return
    doc = update.message.document; # ... (проверка mime) ...
    ...
    try: doc_file = await doc.get_file(); file_bytes = await doc_file.download_as_bytearray(); # ... (проверка на пустой после скачивания) ...
    ...
    await context.bot.send_chat_action(chat_id=chat_id, action=ChatAction.TYPING)
    # ... (определение кодировки, декодирование, проверка на None и пустой текст) ...
    text = None; detected_encoding = None; encodings_to_try = ['utf-8-sig', 'utf-8', 'cp1251', 'latin-1', 'cp866', 'iso-8859-5'] # ... (логика chardet) ...
    ...
    # ... (обрезка текста) ...
    MAX_FILE_CHARS = min(MAX_CONTEXT_CHARS // 2, MAX_OUTPUT_TOKENS * 8); # Упрощенный лимит
    truncated = text; warning_msg = "";
    ...
    # ... (вызов handle_message через fake_update) ...
    if hasattr(update.message, 'reply_text') and callable(update.message.reply_text):
        fake_message = type('obj', (object,), {'text': user_prompt, 'reply_text': update.message.reply_text, 'chat_id': chat_id}) # Без file_id
...
# ... (setup_bot_and_server, run_web_server, handle_telegram_webhook, main) ...
async def setup_bot_and_server(stop_event: asyncio.Event): # ... (код как в предыдущем ответе) ...
...
async def run_web_server(application: Application, stop_event: asyncio.Event): # ... (код как в предыдущем ответе) ...
    app = aiohttp.web.Application(); async def health_check(request): # ... (health check) ...
...
async def handle_telegram_webhook(request: aiohttp.web.Request) -> aiohttp.web.Response: # ... (код как в предыдущем ответе) ...
    application = request.app.get('bot_app'); # ... (проверка application и secret token) ...
    if not application: logger.critical("Приложение бота не найдено!"); return aiohttp.web.Response(status=500)
    secret_token = os.getenv('WEBHOOK_SECRET_TOKEN'); # ... (проверка токена) ...
...
async def main(): # ... (код как в предыдущем ответе) ...
    log_level_str = os.getenv("LOG_LEVEL", "INFO").upper(); log_level = getattr(logging, log_level_str, logging.INFO); logging.basicConfig(format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.INFO); # ... (установка уровней логов) ...
    ...
    loop = asyncio.get_running_loop(); stop_event = asyncio.Event(); # ... (обработка сигналов) ...
    def signal_handler(): # ...
    ...
    finally: # ... (логика штатной остановки) ...
        logger.info("--- Начало остановки ---"); # ... (остановка веб-сервера, shutdown, закрытие сессии, отмена задач) ...
        if not stop_event.is_set(): stop_event.set(); # ... (остановка веб-сервера с wait_for/cancel) ...
        if web_server_task and not web_server_task.done(): logger.info("Остановка веб-сервера..."); try: await asyncio.wait_for(web_server_task, timeout=15.0); logger.info("Веб-сервер завершен.") except asyncio.TimeoutError: logger.warning("Таймаут остановки веб-сервера, отмена..."); web_server_task.cancel(); try: await web_server_task except asyncio.CancelledError: logger.info("Веб-сервер отменен.") except Exception as e_cancel: logger.error(f"Ошибка отмены веб-сервера: {e_cancel}", exc_info=True) except asyncio.CancelledError: logger.info("Ожидание веб-сервера отменено.") except Exception as e_wait: logger.error(f"Ошибка ожидания веб-сервера: {e_wait}", exc_info=True)
        if application: logger.info("Остановка приложения Telegram..."); try: await application.shutdown(); logger.info("Приложение Telegram остановлено.") except Exception as e_shutdown: logger.error(f"Ошибка shutdown(): {e_shutdown}", exc_info=True)
        if aiohttp_session_main and not aiohttp_session_main.closed: logger.info("Закрытие сессии aiohttp..."); await aiohttp_session_main.close(); await asyncio.sleep(0.5); logger.info("Сессия aiohttp закрыта.")
        tasks = [t for t in asyncio.all_tasks() if t is not asyncio.current_task()]; # ... (отмена оставшихся задач) ...
        if tasks: logger.info(f"Отмена {len(tasks)} задач..."); [task.cancel() for task in tasks]; results = await asyncio.gather(*tasks, return_exceptions=True); # ... (логирование результатов отмены) ...
