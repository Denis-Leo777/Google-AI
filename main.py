...
try:
    ...
    logger.error(f"!!! НЕ удалось импортировать модуль google.genai.types: {e}. Используются заглушки.")
...
def extract_response_text(response) -> Optional[str]:
    try:
        ...
        try:
            ...
            logger.warning(f"Ошибка при получении деталей ошибки: {e_details}")
            return None
    except AttributeError:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_html(
        rf"Привет, {user.mention_html()}! Бот Gemini (client) vWebhook."
...
async def select_model_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for alias in AVAILABLE_MODELS.keys():
        keyboard.append([InlineKeyboardButton(f"✅ {alias}" if alias == current_alias else alias, callback_data=alias)])
    if not keyboard:
...
async def select_model_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if selected_alias == current_alias:
        ...
        try:
            ...
            logger.warning(f"Не удалось изменить разметку: {e}")
        return
    ...
    try:
        await query.edit_message_text(
            text=f"✅ Модель: *{selected_alias}*!\n\nНачните чат:",
            reply_markup=reply_markup,
            parse_mode=ParseMode.MARKDOWN
        )
    except Exception as e:
        logger.warning(f"Не удалось изменить сообщение: {e}")
        await context.bot.send_message(chat_id=chat_id, text=f"Модель: *{selected_alias}*!", parse_mode=ParseMode.MARKDOWN)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not model_id:
        logger.error(f"Крит. ошибка: Не найден ID для '{selected_alias}'")
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Ошибка Part user: {e}")
        ...
        logger.info(f"Ответ от '{model_id}' получен за {processing_time:.2f} сек.")
            ...
            try:
                ...
                if grounding_metadata:
                    ...
                    if web_queries:
                        ...
                        logger.info(f"Найдены webSearchQueries ({len(search_suggestions)}): {search_suggestions}")
                ...
                if citation_metadata and hasattr(citation_metadata, 'citation_sources'):
                    ...
                    urls = [s.uri for s in sources if hasattr(s, 'uri') and s.uri]
                    if urls:
                        logger.info(f"Найдены источники ({len(urls)}).")
                        [search_suggestions.append(url) for url in urls if url not in search_suggestions]
    ...
    if search_suggestions:
        ...
        for suggestion in search_suggestions[:4]:
            if suggestion.startswith('http'):
                try:
                    domain = urllib.parse.urlparse(suggestion).netloc or suggestion[:30] + ".."
                except Exception:
                    domain = suggestion[:30] + ".."
                keyboard.append([InlineKeyboardButton(f"🔗 {domain}", url=suggestion)])
            else:
                try:
                    encoded = urllib.parse.quote_plus(suggestion)
                    url = f"https://google.com/search?q={encoded}"
                    keyboard.append([InlineKeyboardButton(f"🔍 {suggestion}", url=url)])
                except Exception as e:
                    logger.error(f"Ошибка кодирования запроса: {e}")
        if keyboard:
            reply_markup = InlineKeyboardMarkup(keyboard)
            logger.info(f"Добавлена клавиатура с {len(keyboard)} ссылками/запросами.")
    if final_text:
        max_length = 4096
        bot_response = final_text
        if len(bot_response) > max_length:
            logger.warning(f"Ответ >{max_length}, обрезаем.")
        ...
        try:
            ...
            logger.info(f"Отправлен ответ ({len(bot_response)} симв.).")
        except Exception as e:
        ...
        try:
            ...
            logger.error(f"Не удалось отправить ошибку Telegram: {e}")
    else:
        logger.warning("Нет ни текста, ни ошибки.")
        try:
            await update.message.reply_text("Модель вернула пустой ответ без ошибок. 🤷", reply_to_message_id=message_id)
        except Exception as e:
            logger.error(f"Не удалось отправить fallback ответ: {e}")
...
async def setup_bot() -> Optional[Application]:
    ...
    try:
        if 'gemini_client' not in globals() or not gemini_client:
            raise RuntimeError("Клиент Gemini не создан.")
        if not TELEGRAM_BOT_TOKEN:
            raise RuntimeError("Токен Telegram не найден.")
        if not GOOGLE_API_KEY:
            raise RuntimeError("Ключ Google API не найден.")
        ...
        # Добавляем TypeHandler для всех обновлений (group=-1)
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.info("Прерывание (KeyboardInterrupt/SystemExit) получено. Завершаем работу.")
    except Exception as e:
