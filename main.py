# --- START OF REALLY x30 FULL CORRECTED main.py (AIOHTTP INTEGRATED) ---
...
import aiohttp.web # <-- ДОБАВЛЕН ИМПОРТ
...
def extract_response_text(response) -> Optional[str]:
    ...
    except ValueError as e_val: logger.warning(f"ValueError: {e_val}"); # ... (обработка ошибок)
    except AttributeError: logger.warning(".text нет, пробуем parts."); # ... (обработка ошибок)
...
async def run_web_server(port: int):
    ...
    app.router.add_get('/', handle_ping) # Отвечаем на запросы к корню
    runner = aiohttp.web.AppRunner(app)
    await runner.setup()
    # Слушаем на всех доступных интерфейсах (0.0.0.0) указанный порт
    site = aiohttp.web.TCPSite(runner, '0.0.0.0', port)
    try:
        await site.start()
        logger.info(f"Веб-сервер для пинга запущен на http://0.0.0.0:{port}")
        # Бесконечный цикл ожидания, пока основная задача (бот) работает
        while True:
            await asyncio.sleep(3600) # Просыпаемся раз в час, чтобы проверить жив ли еще
    except Exception as e:
        logger.exception(f"Ошибка в работе веб-сервера: {e}")
    finally:
        # Корректно останавливаем сервер при завершении
...
async def main_async() -> None:
    ...
    # Для локального тестирования можно использовать, например, 8080
    port = int(os.environ.get("PORT", 8080))
    logger.info(f"Порт для веб-сервера: {port}")

    # --- Запускаем обе задачи параллельно ---
    logger.info("Запуск Telegram бота (polling) и веб-сервера для пинга...")
    try:
        # run_polling сам по себе блокирующий в некотором смысле,
        # но Application().run_polling запускает свой event loop или интегрируется
        ...
        # Ловим ошибки на уровне gather, если одна из задач упадет
        logger.exception(f"Критическая ошибка в asyncio.gather: {e}")
    finally:
        # Попытка корректно остановить приложение Telegram при завершении
...
if __name__ == '__main__':
    if 'gemini_client' in globals() and gemini_client:
        ...
        try:
            ...
            # Обрабатываем Ctrl+C или сигнал завершения
...
# --- END OF REALLY x30 FULL CORRECTED main.py (AIOHTTP INTEGRATED) ---
