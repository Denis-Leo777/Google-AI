...
# import serpapi  # Раскомментируй, если будешь использовать SerpAPI
...
# SERPAPI_KEY = os.getenv('SERPAPI_KEY')  # Добавь в .env, если используешь
...
# async def serpapi_search(query):  # Раскомментируй для SerpAPI
#     try:
#         client = serpapi.Client(api_key=SERPAPI_KEY)
#         results = client.search(q=query, engine="google")
#         snippets = [r.get('snippet', '') for r in results.get('organic_results', [])[:3]]
#         links = [r.get('link', '') for r in results.get('organic_results', [])[:3]]
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # if use_search and not tools:  # Fallback на SerpAPI
        #     snippets, links = await serpapi_search(user_message)
        ...
        sources = []
        if use_search and hasattr(response, 'citationMetadata'):
