...
async def clear_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.chat_data['history'] = [{"role": "system", "parts": [{"text": system_instruction_text}]}]  # Исправлено
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat_history = context.chat_data.setdefault("history", [{"role": "system", "parts": [{"text": system_instruction_text}]}])  # Исправлено
    ...
    try:
        ...
        tools = [
            genai.Tool(
                ...
                google_search_retrieval=genai.types.gapic.GoogleSearchRetrieval()  # Исправлено
            )
        ...
        reply = response.text[:1500] or "🤖 Нет текстового ответа от модели."  # Добавлена обрезка
        chat_history.extend([
            {'role': 'user', 'parts': [{'text': user_message}]},  # Исправлено
            {'role': 'model', 'parts': [{'text': reply}]}  # Исправлено
        ])

        # Ограничение контекста
        total_chars = sum(len(p['parts'][0]['text']) for p in chat_history)  # Исправлено
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        model_id = user_selected_model.get(chat_id, DEFAULT_MODEL)  # Исправлено
        ...
        reply = response.text[:1500] or "🤖 Не удалось проанализировать изображение."  # Добавлена обрезка
    except Exception as e:
...
if __name__ == '__main__':  # Исправлено
    ...
    try:
        ...
        if 'application' in locals():  # Добавлена проверка
