...
"Если спрашивают про 'сейчас', 'сегодня', 'завтра', 'вчера', и подобные близкие периоды времени, то ВСЕГДА ОБЯЗАТЕЛЬНО сначала проверяй текущую дату и время от серверов времени в интернете. Затем проводи поиск свежей информации в интернете, исходя из этой даты."
...
"Никогда не сокращай и не редактируй используемый в чате текст или код без запроса или разрешения."
"При исправлении ошибки: указывай строку(и) и причину. Бери за основу последнюю ПОЛНУЮ подтверждённую верной версию частей текста, кода и т.д.. Вноси только необходимые изменения, не трогая остальное без запроса. При сомнениях — уточняй. Если ошибка повторяется — веди «список ошибок» для сессии и проверяй эти места. Всегда указывай, на какую версию или сообщение опираешься при правке."
)
# ===================================================
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    start_message = (
        ...
        f"\nGoogle GEMINI {default_model_name}"
        ...
        f"\n- пишите в личку и добавляйте меня в группы, я запоминаю историю чата и понимаю, кто мне пишет."
