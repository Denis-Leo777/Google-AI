...
# ... (все предыдущие фичи)
# + Указание tools строкой ["google_search"]
# + Сравнение finish_reason с 0 (предположительно STOP)
# + Явное указание пути к Tesseract
...
# ===== ИСПРАВЛЕНИЕ: Явно указываем путь к Tesseract =====
try:
    pytesseract.pytesseract.tesseract_cmd = '/usr/bin/tesseract'
    # Проверим доступность Tesseract при старте
    pytesseract.get_tesseract_version()
    logger.info(f"Tesseract найден по пути: {pytesseract.pytesseract.tesseract_cmd}")
except Exception as e:
    logger.error(f"Tesseract НЕ НАЙДЕН по пути '/usr/bin/tesseract' или не работает! Ошибка: {e}. OCR будет недоступен.")
    # Не выходим, просто OCR не будет работать
# ========================================================
...
# ===== ИСПРАВЛЕНИЕ: Убираем ненужные импорты =====
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tools = [] # Инициализируем пустым списком
    if model_id == IMAGE_GEN_MODEL:
        ...
        use_search = False # Поиск не нужен
    else:
        ...
        # ===== ИСПРАВЛЕНИЕ: Указываем tool как строку =====
        if use_search:
            tools = ["google_search"]
    ...
    try:
        ...
        model = genai.GenerativeModel(
            model_id,
            tools=tools, # Передаем либо ["google_search"], либо []
            safety_settings=[],
        ...
        if model_id == IMAGE_GEN_MODEL:
            ...
            if images_sent == 0 and not reply:
                 ...
                 # ===== ИСПРАВЛЕНИЕ: Сравниваем значение finish_reason с 0 (предположительно STOP) =====
                 # У finish_reason может быть и None, поэтому проверяем явно
        ...
        try: # Обернем проверки типов исключений в try-except на случай их отсутствия
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    # ===== ИСПРАВЛЕНИЕ: Проверяем доступность Tesseract перед использованием =====
    tesseract_available = False
    try:
        pytesseract.get_tesseract_version()
        tesseract_available = True
    except Exception:
        logger.error("Tesseract не найден или не настроен в handle_photo. OCR не будет выполнен.")
    # ==========================================================================
    ...
    # Попытка OCR только если Tesseract доступен
    if tesseract_available:
        try:
            ...
            # Продолжаем как обычное изображение

    # Обработка как изображение (если OCR не доступен, не сработал или не нашел текст)
    ...
    try:
        model = genai.GenerativeModel(
            model_id,
            tools=tools, # Передаем ["google_search"] или []
            safety_settings=[],
            generation_config={"temperature": temperature},
