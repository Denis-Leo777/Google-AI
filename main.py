# --- START OF REALLY x31 FULL CORRECTED main.py (FIXED INDENTATION AGAIN) ---
    ...
    try:
        ...
        if hasattr(response, 'candidates') and response.candidates:
             try:
                 ...
                 pass # Ошибки доступа к метаданным игнорируем

    # --- СНОВА ИСПРАВЛЕННЫЕ БЛОКИ EXCEPT ---
    except InvalidArgument as e_arg:
        # Логируем и устанавливаем сообщение об ошибке
        ...
        # Логируем полное исключение и устанавливаем сообщение об ошибке
...
# --- ИЗМЕНЕННАЯ ФУНКЦИЯ MAIN (как в v30) ---
async def main_async() -> None:
    ...
    except Exception as e: logger.exception(f"Критическая ошибка в asyncio.gather: {e}")
    finally:
        if application._is_running: logger.info("Остановка приложения Telegram..."); await application.stop(); await application.shutdown(); logger.info("Приложение Telegram остановлено.")

# --- ТОЧКА ВХОДА (как в v30) ---
if __name__ == '__main__':
    if 'gemini_client' in globals() and gemini_client:
        logger.info("Клиент Gemini создан. Запускаем основной цикл asyncio.")
        try: asyncio.run(main_async())
        except (KeyboardInterrupt, SystemExit): logger.info("Получен сигнал завершения. Бот останавливается...")
...
# --- END OF REALLY x31 FULL CORRECTED main.py (FIXED INDENTATION AGAIN) ---
