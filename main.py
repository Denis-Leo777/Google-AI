...
# ... (все предыдущие фичи)
# + Попытка импорта FinishReason из google.generativeai.types
...
try:
    # Tool и Retrieval из одного места
    from google.ai.generativelanguage_v1beta.types import Tool, GoogleSearchRetrieval
    # ===== ИСПРАВЛЕНИЕ: Пробуем FinishReason из другого места =====
    from google.generativeai.types import FinishReason
    logger.info("Успешно импортированы Tool, GoogleSearchRetrieval из google.ai... и FinishReason из google.generativeai.types")
except ImportError as e:
    logger.critical(f"Не удалось импортировать необходимые классы! Ошибка: {e}")
    ...
    try:
        ...
        if model_id == IMAGE_GEN_MODEL:
            ...
            if images_sent == 0 and not reply:
                 ...
                 try:
                     feedback = response.prompt_feedback # Получаем объект обратной связи
                     block_reason = feedback.block_reason if feedback else None # Проверяем наличие feedback
                     # ===== ИСПРАВЛЕНИЕ: Проверяем наличие candidates перед доступом =====
                     if response.candidates:
                        finish_reason_val = response.candidates[0].finish_reason
                 except AttributeError: # Ловим если полей нет вообще
                      logger.warning(f"ChatID: {chat_id} | Не удалось получить prompt_feedback или candidates/finish_reason из ответа.")
                 except Exception as e_fb: # Ловим другие возможные ошибки
                 ...
                 # ===== ИСПРАВЛЕНИЕ: Используем импортированный FinishReason.STOP =====
