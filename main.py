...
# - ИСПРАВЛЕНО: SyntaxError в send_reply и select_model_callback (try/except на новых строках).
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    start_message = (
        f"Google GEMINI **{default_model_name}**" # <-- ** внутри f-строки
    ...
    if callback_data and callback_data.startswith("set_model_"):
        ...
        if selected in AVAILABLE_MODELS:
            ...
            try:
                ...
                # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< ИСПРАВЛЕНО: Разделение строки else <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                else:
        ...
        try: await query.edit_message_text("❌ Ошибка выбора.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    while current_total_chars > MAX_CONTEXT_CHARS and len(chat_history) > 1: # Обрезка...
        removed_entry = chat_history.pop(0); # ... (уменьшение current_total_chars) ...; removed_count += 1
        if chat_history: removed_entry = chat_history.pop(0); # ... ; removed_count += 1
        ...
        try:
            ...
            if chardet_limit > 0:
                 ...
                 if detected and detected['encoding'] and detected['confidence'] > 0.7:
                      ...
                      if potential_encoding == 'utf-8' and file_bytes.startswith(b'\xef\xbb\xbf'): logger.info(f"ChatID: {chat_id} | UTF-8 BOM -> 'utf-8-sig'."); detected_encoding = 'utf-8-sig'; # ... (обновление encodings_to_try) ...
                      else: detected_encoding = potential_encoding; # ... (обновление encodings_to_try) ...
...
async def handle_telegram_webhook(request: aiohttp.web.Request) -> aiohttp.web.Response:
    ...
    if secret_token: header_token = request.headers.get('X-Telegram-Bot-Api-Secret-Token'); # ... (проверка токена) ...
...
--- END OF FILE main.txt ---```{warning_msg}"
    user_prompt = f"Файл `{file_name}` с комм: \"{user_caption}\". {file_context}\nПроанализируй." if user_caption else f"Файл `{file_name}`. {file_context}\nЧто скажешь?"
    if hasattr(update.message, 'reply_text') and callable(update.message.reply_text):
        fake_message = type('obj', (object,), {'text': user_prompt, 'reply_text': update.message.reply_text, 'chat_id': chat_id})
        fake_update = type('obj', (object,), {'effective_chat': update.effective_chat, 'message': fake_message}); await handle_message(fake_update, context)
