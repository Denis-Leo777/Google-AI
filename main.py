...
# ... (все предыдущие фичи)
# + Исправлена инициализация Tool через словарь
# + Упрощен запрос для Image Gen, добавлено логгирование ответа
...
import io
import pprint # Для красивого логгирования ответа модели
...
# Импорты для Tool
try:
    # Tool нужен для структуры, GoogleSearchRetrieval - для объекта внутри
...
# ===== ИСПРАВЛЕНИЕ: Обновляем список моделей =====
AVAILABLE_MODELS = {
    'gemini-2.5-pro-exp-03-25': '2.5 Pro (Exp)',
    'gemini-2.0-flash-001': '2.0 Flash', # Обновлено имя модели
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    start_message = (
        ...
        f"\nВы можете пользоваться Google-поиском, улучшенными настройками, чтением изображений и текстовых файлов, а также **генерацией картинок**."
        "\n/model — выбор модели,"
        "\n/clear — очистить историю."
        "\n/temp <0-2> — установить температуру (креативность)."
        "\n/search_on /search_off — вкл/выкл Google Поиск (не работает для Image Gen)."
...
async def select_model_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    selected = query.data
    if selected in AVAILABLE_MODELS: # Проверка по ключам словаря
        user_selected_model[chat_id] = selected
        model_name = AVAILABLE_MODELS[selected]
        reply_text = f"Модель установлена: **{model_name}**"
        if selected == IMAGE_GEN_MODEL:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if model_id == IMAGE_GEN_MODEL:
        ...
        # ===== ИСПРАВЛЕНИЕ: Убираем system_instruction для Image Gen =====
        ...
        # ===== ИСПРАВЛЕНИЕ: Инициализация Tool через словарь =====
        if use_search:
            try:
                # Создаем словарь для инициализации Tool
                # Пробуем поле 'google_search_retrieval', как чаще всего в доках/примерах
                tool_dict = {'google_search_retrieval': GoogleSearchRetrieval()}
                # Если предыдущая ошибка была права, то надо 'google_search': GoogleSearchRetrieval()
                # tool_dict = {'google_search': GoogleSearchRetrieval()} # Запасной вариант
                tools = [Tool(tool_dict)] # Инициализируем Tool этим словарем
                ...
                tools = []
                use_search = False # Отключаем поиск, если не смогли создать инструмент
        else:
            tools = []
        # =======================================================
    ...
    try:
        ...
        # Если это image gen, может нужны другие параметры? Пока нет.
        ...
        # ===== ИСПРАВЛЕНИЕ: Логгирование полного ответа Image Gen =====
        if model_id == IMAGE_GEN_MODEL:
            # Логируем структуру ответа для отладки
            logger.debug(f"ChatID: {chat_id} | Полный ответ от Image Gen модели:\n{pprint.pformat(response)}")

            images_sent = 0
            # Проверяем стандартный путь к данным картинки
            if response.candidates and response.candidates[0].content and response.candidates[0].content.parts:
                for part in response.candidates[0].content.parts:
                    # Ищем именно inline_data с типом image
                    if part.inline_data and part.inline_data.mime_type.startswith("image/"):
                        ...
                        try:
                            ...
                            images_sent += 1
                            # Пока отправляем только первую найденную картинку
                            break # Выходим из цикла for part
            ...
            if images_sent == 0 and not reply: # Если картинку не отправили и ошибки отправки не было
                 ...
                 except Exception: pass # Игнорим ошибки доступа к полям
                 ...
                 elif finish_reason and finish_reason != genai.types.FinishReason.STOP:
                     logger.warning(f"ChatID: {chat_id} | Генерация изображения завершилась не штатно. Причина: {finish_reason}")
                     reply = f"❌ Не удалось сгенерировать изображение. Причина: {finish_reason}"
                 else:
        ...
        error_message = str(e)
        if isinstance(e, genai.types.BlockedPromptException): # Явно ловим блокировку
             reply = f"❌ Запрос заблокирован моделью. Причина: {e}"
        elif isinstance(e, genai.types.StopCandidateException): # Явно ловим остановку
             reply = f"❌ Генерация остановлена моделью. Причина: {e}"
        elif "Message must be initialized with a dict" in error_message:
             reply = f"❌ Ошибка конфигурации: Неправильный формат объекта Tool. Разработчик чинит..." # Скрываем детали от юзера
...
# остаются БЕЗ ИЗМЕНЕНИЙ по сравнению с версией из сообщения #13 (где мы меняли поле на google_search)
# Важно: Убедись, что в handle_photo ИНИЦИАЛИЗАЦИЯ tools происходит так же, как в handle_message (через словарь)
# ... (код этих функций ниже с исправленной инициализацией tools в handle_photo) ...
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== ИСПРАВЛЕНИЕ: Инициализация Tool через словарь (как в handle_message) =====
    if use_search:
        try:
            tool_dict = {'google_search_retrieval': GoogleSearchRetrieval()}
    ...
    if reply: # Отправляем текстовый ответ ТОЛЬКО если он есть
        await update.message.reply_text(reply, parse_mode='Markdown')
...
async def main():
    # Устанавливаем уровень логгирования DEBUG для google библиотек, чтобы видеть больше деталей
