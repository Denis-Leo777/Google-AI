...
GOOGLE_CSE_ID = os.getenv('GOOGLE_CSE_ID')  # ID вашей поисковой системы
...
# Функция для выполнения запроса к Google Custom Search API
async def google_custom_search(query: str) -> list:
    url = "https://www.googleapis.com/customsearch/v1"
    params = {
        "key": GOOGLE_API_KEY,
        "cx": GOOGLE_CSE_ID,
        "q": query,
        "num": 30  # максимум 10 результатов за запрос
    }
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params) as resp:
                if resp.status == 200:
                    ...
                    snippets = [f"- {item.get('snippet', '')}" for item in items if item.get('snippet')]
                    return snippets
                else:
                    logger.error(f"Ошибка Google Custom Search: статус {resp.status}")
                    return []
    except Exception as e:
        logger.error(f"Исключение при запросе к Google Custom Search: {e}")
        return []
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    start_message = (
        ...
        f"\n + поиск Google, улучшенные настройки точности, логики и юмора, чтение изображений (OCR) и текстовых файлов"
        "\n/model — выбор модели"
...
async def enable_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🦆 Поиск Google включён.")

async def disable_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_search_enabled[update.effective_chat.id] = False
    await update.message.reply_text("🔇 Поиск Google отключён.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if use_search:
        logger.info(f"ChatID: {chat_id} | Поиск Google включён. Запрос: '{original_user_message[:50]}...'")
        # Сначала пробуем Google Custom Search
        snippets = await google_custom_search(original_user_message)
        if snippets:
            ...
            logger.info(f"ChatID: {chat_id} | Найдены результаты Google: {len(snippets)} сниппетов.")
        else:
            logger.info(f"ChatID: {chat_id} | Результаты Google не найдены или произошла ошибка, используем DuckDuckGo.")
            try:
                ddgs = DDGS()
                ...
                if results:
                    ...
                    if search_snippets:
                        ...
                        logger.info(f"ChatID: {chat_id} | Найдены результаты DDG: {len(search_snippets)} сниппетов.")
            except Exception as e_ddg:
                logger.error(f"ChatID: {chat_id} | Ошибка при поиске DuckDuckGo: {e_ddg}", exc_info=True)
    else:
        logger.info(f"ChatID: {chat_id} | Поиск отключён.")

    logger.debug(f"ChatID: {chat_id} | Финальный промпт для Gemini:\n{final_user_prompt}")
    logger.info(f"ChatID: {chat_id} | Модель: {model_id}, Темп: {temperature}, Поиск: {'Контекст добавлен' if search_context else 'Контекст НЕ добавлен'}")
