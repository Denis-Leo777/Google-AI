...
# - ИСПРАВЛЕНО: Множественные SyntaxError (if/try после ; в else/except/в коде).
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if reply:
        if message: await send_reply(message, reply, context)
            ...
            logger.error(f"ChatID: {chat_id} | Нет сообщения для ответа.")
            try:
                await context.bot.send_message(chat_id=chat_id, text=reply)
            except Exception as e_send_direct:
                logger.error(f"ChatID: {chat_id} | Не удалось отправить напрямую: {e_send_direct}")
    else:
         logger.error(f"ChatID: {chat_id} | Нет ответа для отправки.")
         try:
             await message.reply_text("🤖 Не удалось получить ответ.")
         except Exception as e_final_fail:
             logger.error(f"ChatID: {chat_id} | Не удалось отправить финальную ошибку: {e_final_fail}")
# =============================================================
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        pass # Игнорируем ошибки при проверке версии Tesseract
    # ==========================================================================
    ...
    if not ocr_triggered: # --- Обработка как изображение ---
        ...
        if reply_to_send: await send_reply(message, reply_to_send, context)
            ...
            logger.error(f"ChatID: {chat_id} | Vision нет ответа для отправки.")
            try:
                await message.reply_text("🤖 Не удалось описать.")
            except Exception as e_final_fail:
                logger.error(f"ChatID: {chat_id} | Vision не удалось отправить ошибку: {e_final_fail}")
# =================================================================
...
async def main():
    ...
    for sig in (signal.SIGINT, signal.SIGTERM):
        try:
            ...
            logger.warning(f"Не удалось установить обработчик {sig} через loop.")
            try: # Попытка установить через signal.signal как fallback
                signal.signal(sig, lambda s, f: signal_handler())
            except Exception as e_signal:
                logger.error(f"Не удалось установить обработчик {sig}: {e_signal}")
    # ==========================================================================
        ...
        if web_server_task and not web_server_task.done(): # Остановка веб-сервера
             ...
             try: await asyncio.wait_for(web_server_task, timeout=15.0); logger.info("Веб-сервер завершен.")
                 ...
                 try:
                     ...
                     logger.info("Веб-сервер отменен.")
                 except Exception as e_cancel:
                     logger.error(f"Ошибка отмены веб-сервера: {e_cancel}", exc_info=True)
