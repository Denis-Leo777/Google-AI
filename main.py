...
# ... (все предыдущие фичи)
# + gemini-2.5-pro-exp-03-25 СНОВА без поиска
# + Исправлена инициализация Tool через словарь с ключом 'google_search'
# + Исправлен импорт и использование FinishReason
# + Убрана Markdown разметка из сообщений об ошибках
...
# Импорты для Tool и FinishReason
try:
    # ===== ИСПРАВЛЕНИЕ: Добавляем FinishReason =====
    from google.ai.generativelanguage_v1beta.types import Tool, GoogleSearchRetrieval, FinishReason
    logger.info("Успешно импортированы Tool, GoogleSearchRetrieval, FinishReason из google.ai.generativelanguage_v1beta.types")
except ImportError as e:
    logger.critical(f"Не удалось импортировать необходимые классы Tool/GoogleSearchRetrieval/FinishReason! Ошибка: {e}")
...
# Обновляем список моделей согласно твоему запросу
AVAILABLE_MODELS = {
...
# ===== ИСПРАВЛЕНИЕ: Возвращаем 2.5 Pro в список "без поиска" =====
...
# Команды start, clear_history, set_temperature, enable_search, disable_search, model_command ...
# Обновляем тексты сообщений согласно тому, что 2.5 Pro не поддерживает поиск
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    start_message = (
        ...
        "\n/search_on /search_off — вкл/выкл Google Поиск (не работает для Image Gen и 2.5 Pro Exp)." # Уточнили
        "\nКанал автора: t.me/denisobovsyom"
    )
    if DEFAULT_MODEL in MODELS_WITHOUT_SEARCH: # Проверка вернется, т.к. 2.5 Pro опять в списке
...
async def enable_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Возвращаем проверку, т.к. 2.5 Pro снова не поддерживает
...
async def model_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for m, name in AVAILABLE_MODELS.items():
         ...
         # Возвращаем пометку для всех моделей без поиска
...
async def select_model_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if selected in AVAILABLE_MODELS:
        ...
        # Возвращаем проверку для всех неподдерживающих
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if model_id == IMAGE_GEN_MODEL:
        ...
        # Логика поиска теперь снова учитывает MODELS_WITHOUT_SEARCH
        ...
        if use_search:
            try:
                # ===== ИСПРАВЛЕНИЕ: Используем ключ 'google_search' =====
                ...
                # ===== ИСПРАВЛЕНИЕ: Убираем parse_mode из ошибки =====
    ...
    try:
        ...
        if model_id == IMAGE_GEN_MODEL:
            ...
            if images_sent == 0 and not reply:
                 block_reason = None
                 finish_reason_val = None # Переименовали, чтобы не конфликтовать с классом FinishReason
                 try:
                     block_reason = response.prompt_feedback.block_reason
                     # ===== ИСПРАВЛЕНИЕ: Используем импортированный FinishReason =====
                 ...
                 # ===== ИСПРАВЛЕНИЕ: Сравниваем с импортированным FinishReason.STOP =====
                 elif finish_reason_val and finish_reason_val != FinishReason.STOP:
                     logger.warning(f"ChatID: {chat_id} | Генерация изображения завершилась не штатно. Причина: {finish_reason_val}")
                     # ===== ИСПРАВЛЕНИЕ: Убираем parse_mode из ошибки =====
            ...
            if not reply:
                try:
                    ...
                    finish_reason_val = candidates_info[0].finish_reason if candidates_info else 'N/A' # Используем другое имя переменной
        ...
        if isinstance(e, genai.types.BlockedPromptException):
            ...
            # Используем model_id, а не AVAILABLE_MODELS.get() чтобы избежать Markdown в имени
    ...
    if reply:
        # ===== ИСПРАВЛЕНИЕ: Отправляем ЛЮБОЙ reply (включая ошибки) без Markdown =====
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== ИСПРАВЛЕНИЕ: Инициализация Tool через словарь с ключом 'google_search' =====
    if use_search:
        try:
            tool_dict = {'google_search': GoogleSearchRetrieval()}
            tools = [Tool(tool_dict)]
    ...
    try:
        ...
        if not reply:
            try:
                ...
                candidates_info = response.candidates
                block_reason = feedback.block_reason if feedback else 'N/A'
                finish_reason_val = candidates_info[0].finish_reason if candidates_info else 'N/A' # Другое имя
                safety_ratings = feedback.safety_ratings if feedback else []
                safety_info = ", ".join([f"{s.category.name}: {s.probability.name}" for s in safety_ratings])
