...
# ===== НОВОЕ: Приоритет диалога над поиском =====
"Приоритезируй прямой диалог с пользователем и его явные утверждения (например, 'у меня ПХД'). Используй информацию из поиска только как вспомогательную, если она релевантна *основному* запросу пользователя, и игнорируй её, если она противоречит диалогу или кажется неуместной."
# ==============================================
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== ИЗМЕНЕНИЕ: Формируем финальный промпт для модели =====
    ...
    for attempt in range(RETRY_ATTEMPTS):
        try:
            ...
            if is_retryable and attempt < RETRY_ATTEMPTS - 1:
                ...
                elif "400" in error_message and ("image input" in error_message or " richiesto" in error_message):
...
# ===== Обработчики фото и документов (без изменений) =====
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_vision_model:
         ...
         if vision_models:
             ...
             fallback_model_id = vision_models[0] # Берем первую попавшуюся
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if file_size_bytes == 0:
         ...
         # return # Пропускаем пустые файлы молча
    ...
    if chardet_available:
        try:
            ...
            if chardet_limit > 0: # Проверяем, что файл не пустой
                 ...
                 if detected and detected['encoding'] and detected['confidence'] > 0.6:
                      ...
                      if detected_encoding == 'utf-8' and file_bytes.startswith(b'\xef\xbb\xbf'):
                          ...
                          # Добавляем utf-8-sig как приоритетный, если еще не пробовали
    ...
    for encoding in list(dict.fromkeys(encodings_to_try)):
        try:
            ...
            if not file_bytes:
                 text = ""
                 logger.info(f"ChatID: {chat_id} | Файл '{doc.file_name}' пустой, пропущен.")
                 break
    ...
    if text is None: # Если цикл завершился без break
    ...
    elif not text and file_size_bytes > 0: # Если файл не пустой, но текст после декодирования пустой (???)
         logger.warning(f"ChatID: {chat_id} | Файл '{doc.file_name}' не пустой, но текст после декодирования пуст.")
         # Можно отправить сообщение об ошибке или пропустить
         await update.message.reply_text(f"⚠️ Не удалось извлечь текст из файла '{doc.file_name}', хотя он не пустой.")
         return
    elif not text and file_size_bytes == 0:
         # Если файл был пустой изначально, пропускаем его молча или уведомляем
...
async def run_web_server(application: Application, stop_event: asyncio.Event):
    ...
    async def health_check(request):
        try:
            # Добавим проверку соединения с Google API (опционально, может замедлять)
            # model = genai.GenerativeModel(DEFAULT_MODEL) # Создаем модель для теста
            # await asyncio.to_thread(model.generate_content, "test") # Пробный короткий запрос
            bot_info = await application.bot.get_me()
            if bot_info:
                 return aiohttp.web.Response(text=f"OK: Bot {bot_info.username} is running. Gemini connection OK (implicitly tested).")
            else:
...
async def handle_telegram_webhook(request: aiohttp.web.Request) -> aiohttp.web.Response:
    ...
    try:
        ...
        # Используем shield для защиты от отмены во время обработки
...
async def main():
    ...
    logging.basicConfig(
        ...
        level=log_level,
        # Добавим хендлер для вывода в файл, если нужно
    ...
    # Уровень для PTB можно сделать WARNING в продакшене
    ...
    for sig in (signal.SIGINT, signal.SIGTERM):
        try:
            loop.add_signal_handler(sig, signal_handler)
             ...
             logger.warning(f"Не удалось установить обработчик для {sig} (возможно, Windows). Используйте Ctrl+C.")
             try: signal.signal(signal.SIGINT, lambda s, f: signal_handler())
             except Exception as e_signal: logger.error(f"Не удалось установить обработчик SIGINT через signal: {e_signal}")
    ...
    try:
        ...
        logger.info("Приложение настроено и запущено. Нажмите Ctrl+C для остановки.")
        await stop_event.wait()
        ...
        # Сначала останавливаем веб-сервер, чтобы он перестал принимать новые запросы
        if web_server_task and not web_server_task.done():
             logger.info("Остановка веб-сервера (graceful shutdown)...")
             # Даем ему время на завершение
             try:
        ...
        # Затем останавливаем приложение бота
        if application:
        ...
        if aiohttp_session_main and not aiohttp_session_main.closed:
             ...
             await asyncio.sleep(0.5) # Пауза для завершения соединений
        ...
        if tasks:
            ...
            try:
                ...
                # Логируем результаты/ошибки отмененных задач
                cancelled_count = 0
                error_count = 0
                for res in results:
                     if isinstance(res, asyncio.CancelledError):
                          ...
                          error_count += 1
                          logger.warning(f"Ошибка в отмененной задаче: {res}")
...
if __name__ == '__main__':
    # # Для отладки асинхронных проблем
